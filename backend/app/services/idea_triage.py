...
MAX_ARRIVALS_WINDOW = 2000  # Hard cap on _recent_arrivals sliding window
...
class IdeaTriageService:
    ...
    async def _on_idea(self, data: Dict[str, Any]) -> None:
        ...
        # Hard cap to prevent unbounded growth during event storms
        if len(self._recent_arrivals) > MAX_ARRIVALS_WINDOW:
            self._recent_arrivals = self._recent_arrivals[-MAX_ARRIVALS_WINDOW:]
