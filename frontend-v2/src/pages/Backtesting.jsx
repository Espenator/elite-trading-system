  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-secondary mt-1">Run strategy backtests with parameter optimization</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title="Backtest Configuration">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <TextField label="End Date" value={endDate} onChange={e => setEndDate(e.target.value)} />
            </div>
            <TextField label="Asset Universe (CSV)" multiline rows={2} value={assets} onChange={e => setAssets(e.target.value)} />
            <TextField label="Initial Capital" value={capital} onChange={e => setCapital(e.target.value)} />
          </div>
        </Card>
        ...
        <Card title="Parameter Sweeps & Controls">
          <div className="space-y-5">
            <div>
              <label className="text-xs text-secondary mb-2 block">Parameter A (Sensitivity): {paramA}</label>
              <input type="range" min="0" max="100" value={paramA} onChange={e => setParamA(Number(e.target.value))} className="w-full accent-primary" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <TextField label="Parameter B Min" value={paramBMin} onChange={e => setParamBMin(e.target.value)} />
              <TextField label="Parameter B Max" value={paramBMax} onChange={e => setParamBMax(e.target.value)} />
            </div>
            <div>
              <label className="text-xs text-secondary mb-2 block">Run Mode</label>
              <div className="flex items-center gap-6">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="runMode" value="single" checked={runMode === 'single'} onChange={() => setRunMode('single')} className="accent-primary" />
                  <span className="text-sm text-white">Single Run</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="runMode" value="sweep" checked={runMode === 'sweep'} onChange={() => setRunMode('sweep')} className="accent-primary" />
                  <span className="text-sm text-white">Parameter Sweep</span>
                </label>
              </div>
            </div>
            <div className="flex items-center gap-3 pt-2">
              <Button variant="primary" leftIcon={Play} onClick={() => setIsRunning(true)}>Run Backtest</Button>
              <Button variant="secondary" leftIcon={Square} onClick={() => setIsRunning(false)}>Stop Run</Button>
              <Button variant="secondary" leftIcon={Download}>Export Configuration</Button>
            </div>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card title="Results Visualization">
            ...
            <div className="h-40 bg-dark/50 rounded-xl border border-secondary/50 flex items-end px-4 pb-4 gap-1">
              {[40, 42, 38, 45, 50, 48, 55, 60, 58, 65, 70, 68, 75, 80, 78, 85, 90, 88, 95, 98].map((v, i) => (
                <div key={i} className="flex-1 bg-primary/60 rounded-t" style={{ height: `${v}%` }} />
              ))}
            </div>
            <div className="flex justify-center gap-4 mt-3">
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-primary" /><span className="text-xs text-secondary">Equity</span></div>
            </div>
            <div className="h-24 bg-dark/50 rounded-xl border border-secondary/50 mt-4 flex items-start px-4 pt-4 gap-1">
              {[5, 8, 3, 12, 18, 15, 10, 20, 25, 18, 12, 22, 28, 20, 15, 10, 8, 12, 5, 3].map((v, i) => (
                <div key={i} className="flex-1 bg-danger/40 rounded-b" style={{ height: `${v * 3}%` }} />
              ))}
            </div>
            <div className="flex justify-center gap-4 mt-3">
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-danger" /><span className="text-xs text-secondary">Drawdown</span></div>
            </div>
          </Card>
        </div>
        ...
        <Card title="Parallel Run Manager" noPadding>
          <DataTable
            columns={[
              { key: 'id', label: 'Run ID', render: (v) => <span className="font-medium text-white">{v}</span> },
              { key: 'strategy', label: 'Strategy', render: (v) => <span className="text-secondary">{v}</span> },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title="Trade-by-Trade Simulator" noPadding>
          <DataTable
            columns={[
              { key: 'time', label: 'Time', render: (v) => <span className="text-secondary">{v}</span> },
              { key: 'asset', label: 'Asset', render: (v) => <span className="font-medium text-white">{v}</span> },
              { key: 'type', label: 'Type', render: (v) => <Badge variant={v === 'BUY' ? 'success' : 'danger'}>{v}</Badge> },
              { key: 'qty', label: 'QTY', cellClassName: 'text-right' },
              { key: 'price', label: 'Price', cellClassName: 'text-right' },
              { key: 'pnl', label: 'PNL', cellClassName: 'text-right', render: (v) => <span className={v >= 0 ? 'text-success' : 'text-danger'}>{v >= 0 ? '+' : ''}{v}</span> },
            ]}
            data={SAMPLE_TRADES}
          />
        </Card>
        ...
        <Card title="Run History & Export">
          <DataTable
            columns={[
              ...
              { key: 'strategy', label: 'Strategy', render: (v) => <span className="text-white">{v}</span> },
              { key: 'pnl', label: 'PNL', cellClassName: 'text-right', render: (v) => <span className={v >= 0 ? 'text-success' : 'text-danger'}>{v >= 0 ? '+' : ''}${Math.abs(v).toLocaleString()}</span> },
            ]}
            data={RUN_HISTORY}
          />
          <Button variant="secondary" fullWidth leftIcon={Download} className="mt-4">Export All Results</Button>
        </Card>
      </div>
    </div>
  );
