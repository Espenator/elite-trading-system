...
import {
  TrendingUp, DollarSign, Activity, Zap, Brain, BarChart3,
  ArrowUpRight, ArrowDownRight, Eye, Bot, Target, ShieldCheck,
  Clock, LayoutDashboard, Gauge, TrendingDown, Shield, Percent,
  Hash, Flame, RefreshCcw, AlertTriangle, Radio, Newspaper,
  Workflow, Users, ChevronUp, ChevronDown, ChevronRight,
  BarChart2, PieChart, Layers, Thermometer, CircleDot,
  ArrowUp, ArrowDown, Briefcase, LineChart, Cpu, Wifi,
} from "lucide-react";
...
function StatCard({ title, value, change, changeType, icon: Icon, color, onClick }) {
  ...
  const iconColors = {
    ...
    secondary: "text-secondary", warning: "text-warning", danger: "text-danger",
  };
  return (
    <div
...
function KpiMicroCard({ label, value, sub, color, icon: Icon, onClick }) {
  ...
  const textMap = {
    ...
    green: "text-emerald-400", purple: "text-purple-400",
  };
  return (
    <div
...
function EquityCurveLW({ data }) {
  ...
  if (!data || data.length === 0) {
    return <div className="h-[280px] flex items-center justify-center text-secondary text-sm">No equity data available</div>;
  }
...
export default function Dashboard() {
  // --- Real API hooks ---
  const { data: portfolioData, loading: portfolioLoading } = useApi("portfolio", { pollIntervalMs: POLL_MS });
  const { data: signalsData, loading: signalsLoading } = useApi("signals", { pollIntervalMs: POLL_MS });
  const { data: agentsData, loading: agentsLoading } = useApi("agents", { pollIntervalMs: POLL_MS });
  const { data: performanceData } = useApi("performance", { pollIntervalMs: POLL_MS });
  ...
  const equityCurveData = useMemo(() => {
    const curve = portfolioData?.equityCurve || performanceData?.equityCurve || [];
    return curve.map((pt) => ({
      time: typeof pt.time === "string" ? Math.floor(new Date(pt.time).getTime() / 1000) : pt.time,
  ...
  const positions = useMemo(() => {
    ...
    return portfolioData.positions.map((pos) => {
      ...
      if (pnlPct == null && entry && qty) pnlPct = (pnlDollars / (entry * qty)) * 100;
      return { ticker, side: pos.side || "Long", entry, current, qty, pnl: pnlDollars, pnlPct: pnlPct ?? 0 };
    });
  }, [portfolioData]);
  ...
  const signals = useMemo(() => {
    ...
    return signalsData.signals.slice(0, 6).map((sig) => {
      ...
      return {
        ticker: sig.symbol, type: sig.action || sig.type || "Signal",
        score: Math.round(prob * 100), time: sig.timestamp ? new Date(sig.timestamp).toLocaleTimeString() : "now",
      };
    });
  }, [signalsData]);
  ...
  const agents = useMemo(() => {
    ...
    const iconMap = {
      "Market Data Agent": Eye, "Signal Generation Agent": Brain, "ML Learning Agent": Cpu,
      "Sentiment Agent": Bot, "YouTube Knowledge Agent": Bot, "Risk Agent": Shield,
      "Execution Agent": Briefcase, "OpenClaw Agent": Target,
    };
    return agentsData.agents.map((agent) => ({
      name: agent.name, status: agent.status === "running" ? "active" : agent.status,
      tasks: agent.tasks_completed ?? "--", icon: iconMap[agent.name] || Bot,
      uptime: agent.uptime ?? null,
    }));
  }, [agentsData]);

  // --- Computed stats ---
  const portfolioValue = performanceData?.portfolioValue ?? portfolioData?.totalValue ?? 0;
  ...
  const maxDrawdown = riskData?.maxDrawdown ?? performanceData?.maxDrawdown ?? 0;
  const regimeShort = regime === "GREEN" ? "Bull" : regime === "RED" ? "Bear" : regime === "YELLOW" ? "Caution" : "--";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        <StatCard
          title="Portfolio Value" icon={DollarSign} color="success"
          value={portfolioValue ? `$${Number(portfolioValue).toLocaleString(undefined, { maximumFractionDigits: 0 })}` : "--"}
          change={dailyPnL != null ? `${dailyPnL >= 0 ? "+" : ""}$${Number(dailyPnL).toFixed(0)}` : null}
          changeType={dailyPnL >= 0 ? "up" : "down"}
        />
        <StatCard
          title="Daily P&L" icon={TrendingUp} color={dailyPnL >= 0 ? "success" : "danger"}
          value={dailyPnL != null ? `${dailyPnL >= 0 ? "+" : ""}$${Number(dailyPnL).toFixed(0)}` : "--"}
        />
        <StatCard title="Win Rate" icon={Target} color="primary" value={winRate ? `${Number(winRate).toFixed(1)}%` : "--"} />
        <StatCard title="Sharpe" icon={BarChart3} color="secondary" value={sharpe ? Number(sharpe).toFixed(2) : "--"} />
        <StatCard
          title="Market Regime" icon={Gauge} color="primary"
          value={openclawLoading ? "..." : regimeShort}
        />
        <StatCard
          title="Candidates" icon={Target} color="secondary"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-10 gap-2">
        <KpiMicroCard label="Total Return" value={`${totalPnLPct >= 0 ? "+" : ""}${Number(totalPnLPct).toFixed(1)}%`} color={totalPnLPct >= 0 ? "green" : "red"} icon={TrendingUp} />
        <KpiMicroCard label="Max Drawdown" value={`${Number(maxDrawdown).toFixed(1)}%`} color="red" icon={TrendingDown} />
        <KpiMicroCard label="Regime" value={regimeShort} sub={regime ?? "--"} color="cyan" icon={Gauge} />
        <KpiMicroCard label="Candidates" value={String(candidates.length)} sub="OpenClaw" color="green" icon={Target} />
        <KpiMicroCard label="Positions" value={String(positions.length)} sub="Active" color="cyan" icon={Briefcase} />
        <KpiMicroCard label="Signals" value={String(signals.length)} sub="Latest" color="purple" icon={Zap} />
        <KpiMicroCard label="Agents" value={String(agents.length)} sub="Running" color="cyan" icon={Bot} />
        <KpiMicroCard label="Win Rate" value={winRate ? `${Number(winRate).toFixed(0)}%` : "--"} color="green" icon={Target} />
        <KpiMicroCard label="Sharpe" value={sharpe ? Number(sharpe).toFixed(2) : "--"} color="amber" icon={BarChart3} />
        <KpiMicroCard label="System" value={statusData?.status === "ok" ? "Online" : "--"} sub={statusData?.latency ? `${statusData.latency}ms` : ""} color={statusData?.status === "ok" ? "green" : "red"} icon={Wifi} />
      </div>

      {/* === ROW 3: Equity Curve (LW Charts) + Market Regime === */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card title="Equity Curve" subtitle="Portfolio performance" className="lg:col-span-2">
          <EquityCurveLW data={equityCurveData} />
        </Card>
        <Card
          title="Market Regime"
          subtitle={lastScanTs ? `Last scan: ${new Date(lastScanTs).toLocaleString()}` : null}
        >
          <div className="flex flex-wrap items-center gap-4 mb-4">
            <Badge variant={regime === "GREEN" ? "success" : regime === "RED" ? "danger" : "warning"} size="lg">
              {regime ?? "--"}
            </Badge>
            {openclawHealth && (
              <span className="text-xs text-secondary">
                Bridge: {openclawHealth.connected ? "Connected" : "Disconnected"}
                {openclawHealth.candidate_count != null && ` | ${openclawHealth.candidate_count} candidates`}
              </span>
            )}
          </div>
          {regimeMarkdown && (
          ...
          {!regimeMarkdown && !openclawLoading && (
            <p className="text-sm text-secondary">No regime readme available. OpenClaw scan may not have run yet.</p>
          )}
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card
          ...
          action={<Link to="/clawbot" className="text-xs font-medium text-primary hover:text-primary/80 flex items-center gap-1">ClawBot Panel <ArrowUpRight className="w-3 h-3" /></Link>}
        >
          {openclawLoading ? (
            ...
            <ul className="space-y-2">
              {candidates.slice(0, 5).map((c, i) => (
                <li key={c.symbol || c.ticker || i} className="flex items-center justify-between py-2 px-3 rounded-lg bg-secondary/10 hover:bg-secondary/20">
                  <span className="font-medium text-white">{c.symbol || c.ticker}</span>
                  <div className="flex items-center gap-2">
                    {c.sector && <span className="text-[10px] text-secondary">{c.sector}</span>}
        ...
        <Card
          ...
          action={<Link to="/trades" className="text-xs font-medium text-primary hover:text-primary/80 flex items-center gap-1">Trade Execution <ArrowUpRight className="w-3 h-3" /></Link>}
        >
          <DataTable
            columns={[
              { key: "ticker", label: "Symbol" },
              { key: "side", label: "Side" },
              { key: "qty", label: "Qty", render: (v) => (v != null ? Number(v).toLocaleString() : "--") },
              { key: "entry", label: "Entry", render: (v) => (v != null ? `$${Number(v).toFixed(2)}` : "--") },
              { key: "current", label: "Current", render: (v) => (v != null ? `$${Number(v).toFixed(2)}` : "--") },
              { key: "pnl", label: "P&L", render: (v) => {
                const val = Number(v);
                return <span className={val >= 0 ? "text-success" : "text-danger"}>{val >= 0 ? "+" : ""}${val.toFixed(2)}</span>;
              }},
              { key: "pnlPct", label: "%", render: (v) => {
                const val = Number(v);
                return <span className={val >= 0 ? "text-success" : "text-danger"}>{val >= 0 ? "+" : ""}{val.toFixed(2)}%</span>;
              }},
            ]}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card
          ...
          action={<Link to="/signals" className="text-xs font-medium text-primary hover:text-primary/80 flex items-center gap-1">Signal Intelligence <ArrowUpRight className="w-3 h-3" /></Link>}
        >
          {signals.length === 0 ? (
            ...
            <ul className="space-y-2">
              {signals.map((s, i) => (
                <li key={s.ticker || i} className="flex items-center justify-between py-2 px-3 rounded-lg bg-secondary/10">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-white">{s.ticker}</span>
                    <span className="text-[10px] text-secondary uppercase">{s.type}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant={s.score >= 70 ? "success" : s.score >= 50 ? "warning" : "danger"} size="sm">
                      {s.score}%
                    </Badge>
        ...
        <Card
          ...
          action={<Link to="/agents" className="text-xs font-medium text-primary hover:text-primary/80 flex items-center gap-1">Agent Command Center <ArrowUpRight className="w-3 h-3" /></Link>}
        >
          {agents.length === 0 ? (
            ...
            <ul className="space-y-2">
              {agents.map((a, i) => {
                const Icon = a.icon;
                return (
                  <li key={a.name || i} className="flex items-center justify-between py-2 px-3 rounded-lg bg-secondary/10">
                    <div className="flex items-center gap-2">
                      {Icon && <Icon className="w-4 h-4 text-primary" />}
                      <span className="text-sm text-white">{a.name}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      {a.tasks !== "--" && <span className="text-[10px] text-secondary font-mono">{a.tasks} tasks</span>}
                      <Badge variant={a.status === "active" ? "success" : a.status === "error" ? "danger" : "secondary"} size="sm">
                        {a.status}
                      </Badge>
                    </div>
                  </li>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
            {sectors.map((s, i) => (
              <SectorCell
                ...
                momentum={s.momentum || (s.change > 0 ? "up" : s.change < 0 ? "down" : "flat")}
              />
            ))}
          </div>
        </Card>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
        {[
          { to: "/signals", label: "Signals", icon: Zap, color: "text-purple-400" },
          { to: "/risk", label: "Risk Intel", icon: Shield, color: "text-red-400" },
          { to: "/performance", label: "Performance", icon: BarChart3, color: "text-cyan-400" },
          { to: "/ml-insights", label: "ML Brain", icon: Brain, color: "text-emerald-400" },
          { to: "/backtest", label: "Backtest Lab", icon: LineChart, color: "text-amber-400" },
          { to: "/market-regime", label: "Market Regime", icon: Gauge, color: "text-cyan-400" },
        ].map((link) => (
          <Link
            key={link.to}
            to={link.to}
