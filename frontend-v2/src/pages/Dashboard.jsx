...
export default function Dashboard() {
  ...
  const processedSignals = useMemo(() => {
    ...
    // Sorting logic matching the pills
    return merged.sort((a, b) => {
      if (activeSortKey === 'Composite Score') return b.score - a.score;
      if (activeSortKey === 'Kelly Optimal') return b.kellyPercent - a.kellyPercent;
      if (activeSortKey === 'R-Mult') return b.rMultiple - a.rMultiple;
      return b.score - a.score; // default fallback
    });
  }, [signalsData, kellyData, activeSortKey]);
  ...
  // Safe data extraction for single objects
    ...
    <div className="flex flex-col h-screen w-full bg-[#0a0e17] text-[#e5e7eb] font-sans text-[9px] leading-tight overflow-hidden selection:bg-[#00d4ff]/30">
          ...
          <div className="flex flex-col border-b border-[#1e3a5f] bg-[#111827] p-2 gap-2 shrink-0">
            ...
            <div className="flex items-center justify-between text-gray-400 font-mono">
              ...
              <div className="flex items-center gap-3">
                <span className="flex items-center gap-1"><div className="w-1.5 h-1.5 rounded-full bg-red-500"></div> Auto-Exec: OFF</span>
                ...
                color: s.impact > 0 ? 'bg-green-500' : 'bg-red-500', w: `${Math.abs(s.impact) * 500}%`
                  ...
                  <div key={shap.f} className="flex items-center justify-between">
                    ...
                    <div className="flex-1 flex items-center mx-2">
                      {shap.val.startsWith('-') ? (
                         <div className="w-1/2 flex justify-end"><div className={`h-1.5 ${shap.color}`} style={{ width: shap.w }}></div></div>
                      ) : (
                         <div className="w-1/2"></div>
                      )}
                      {shap.val.startsWith('+') && (
                        <div className="w-1/2"><div className={`h-1.5 ${shap.color}`} style={{ width: shap.w }}></div></div>
                      )}
                    </div>
                    <span className={shap.val.startsWith('+') ? 'text-green-400 w-8 text-right' : 'text-red-400 w-8 text-right'}>{shap.val}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <footer className="flex items-center justify-between px-3 py-1.5 bg-[#0a0e17] border-t border-[#1e3a5f] shrink-0 font-mono text-[8px] text-gray-400">
        <div className="flex gap-2">
          <button className="bg-[#1e3a5f] hover:bg-[#1e3a5f]/80 text-white px-2 py-0.5 rounded">Run Scan [F5]</button>
          ...
          <button className="bg-cyan-900 text-cyan-400 px-2 py-0.5 rounded">Exec Top 5</button>
          <button className="bg-amber-900 text-amber-400 px-2 py-0.5 rounded">Flatten</button>
          <button className="bg-red-900 text-red-400 px-2 py-0.5 rounded font-bold">EMERGENCY STOP</button>
        </div>
        <div className="flex items-center gap-3">
      ...
      {/* Global CSS overrides specifically for the dark scrollbars inside this component to match theme */}
