...
export default function TradeExecution() {
  ...
  const [activeSignal] = useState({
    ...
    action: 'LONG',
    entry: 75.50,
    stop: 62.00, // Structural Invalidation
    target1: 89.00, // 1R (1:1)
    target2: 102.50, // 2R (2:1)
    confidence: 92,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-200 p-6 flex flex-col xl:flex-row gap-6">
      ...
      <div className="w-full xl:w-1/4 space-y-6">
        <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6 backdrop-blur-md shadow-lg">
          ...
          <div className="text-5xl font-black text-white mb-2 tracking-widest">{activeSignal.ticker}</div>
          <div className="text-sm text-slate-400 mb-6">Confidence Score: <span className="text-green-400 font-bold">{activeSignal.confidence}%</span></div>
          ...
          <div className="space-y-4 font-mono text-sm">
            ...
            <div className="flex justify-between p-3 bg-red-950/20 rounded border border-red-900/30">
              ...
              <span className="text-red-400 font-bold">${activeSignal.stop.toFixed(2)}</span>
            </div>
            <div className="flex justify-between p-3 bg-green-950/20 rounded border border-green-900/30">
              ...
              <span className="text-green-400 font-bold">${activeSignal.target1.toFixed(2)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6 backdrop-blur-md shadow-lg">
          <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
            <ShieldAlert className="w-5 h-5 text-yellow-500" /> Van Tharp Sizing
          </h3>
          <div className="space-y-3 text-sm">
            <div className="flex justify-between">
              <span className="text-slate-400">Account Size</span>
              <span className="font-mono text-white">${accountSize.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Regime Risk Cap</span>
              <span className="font-mono text-yellow-400">{(regimeRisk * 100).toFixed(1)}%</span>
            </div>
            <div className="flex justify-between border-t border-slate-700/50 pt-3">
              ...
              <span className="font-mono text-red-400 font-bold">-${riskAmount.toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="w-full xl:w-2/5 bg-slate-800/40 border border-slate-700/50 rounded-xl p-6 backdrop-blur-md shadow-lg">
        ...
        <p className="text-slate-400 text-sm mb-6">All 6 questions must be answered YES to unlock execution.</p>

        <div className="space-y-3">
          {[
            { id: 'edgeConfirmed', title: '1. Edge Confirmed?', desc: 'Scanner score > 70? Weekly/Daily/4H aligned? Flow confirms?' },
            { id: 'riskPredefined', title: '2. Risk Predefined?', desc: 'Stop placed below structural invalidation level?' },
            ...
            >
              <div className="mt-0.5">
                {checklist[item.id] ? <CheckCircle2 className="text-blue-500" /> : <XCircle className="text-slate-600" />}
              </div>
              <div>
                <h4 className={`font-bold ${checklist[item.id] ? 'text-white' : 'text-slate-300'}`}>{item.title}</h4>
      ...
      <div className="w-full xl:w-[35%] bg-slate-800/40 border border-slate-700/50 rounded-xl p-6 backdrop-blur-md shadow-lg flex flex-col justify-between">
        <div>
          ...
          <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700/50 mb-6">
            <div className="flex justify-between items-end mb-2">
              ...
              <span className="text-2xl font-bold text-white font-mono">{totalShares.toLocaleString()} <span className="text-sm text-slate-500">shares</span></span>
            </div>
            <div className="flex justify-between items-end">
              <span className="text-slate-400 text-sm">Capital Required</span>
              <span className="text-lg text-slate-300 font-mono">${totalCapitalRequired.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
            </div>
          </div>

          <h4 className="text-sm font-bold text-slate-300 mb-3 uppercase tracking-wider">33-33-34 Scale-In Protocol</h4>
          <div className="space-y-2 font-mono text-sm mb-8">
            <div className="grid grid-cols-12 gap-2 bg-slate-900/40 p-3 rounded border border-slate-700/30 items-center">
              ...
              <div className="col-span-3 text-blue-400 font-bold">{entry1} sh</div>
              <div className="col-span-4 text-white">@ ${activeSignal.entry.toFixed(2)}</div>
              <div className="col-span-3 text-right text-xs text-slate-500">Market Open</div>
            </div>
            <div className="grid grid-cols-12 gap-2 bg-slate-900/40 p-3 rounded border border-slate-700/30 items-center">
              <div className="col-span-2 text-slate-500">33%</div>
              <div className="col-span-3 text-blue-400 font-bold">{entry2} sh</div>
              <div className="col-span-4 text-white">@ Retest</div>
              <div className="col-span-3 text-right text-xs text-slate-500">Limit Ord</div>
            </div>
            <div className="grid grid-cols-12 gap-2 bg-slate-900/40 p-3 rounded border border-slate-700/30 items-center">
              <div className="col-span-2 text-slate-500">34%</div>
              <div className="col-span-3 text-blue-400 font-bold">{entry3} sh</div>
              <div className="col-span-4 text-white">@ Breakout</div>
              <div className="col-span-3 text-right text-xs text-slate-500">Stop Lmt</div>
            </div>
          </div>
        </div>
        ...
        <div>
          {orderStatus === 'IDLE' ? (
            ...
            >
              {allChecksPassed ? <><Send className="w-6 h-6" /> FIRE ENTRY 1</> : <><Lock className="w-6 h-6" /> COMPLETE CHECKLIST</>}
            </button>
          ) : orderStatus === 'ROUTING' ? (
            <div className="w-full py-5 rounded-xl bg-yellow-600/20 border border-yellow-500/50 text-yellow-500 font-black text-xl tracking-widest flex justify-center items-center gap-3 animate-pulse">
              <Activity className="w-6 h-6 animate-spin" /> ROUTING TO BROKER...
            </div>
          ) : (
