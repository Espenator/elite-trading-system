  ...
  if (loading && performanceData.length === 0) {
    return (
      <div className="bg-[#0a0a0f] text-slate-50 min-h-screen flex items-center justify-center">
        <div className="text-cyan-500 text-xl animate-pulse">Loading ML Brain & Flywheel...</div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-[#0a0a0f] text-slate-50 min-h-screen p-8 font-sans">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-4xl font-bold text-slate-50 flex items-center gap-3">
            <span className="text-cyan-500">{'\u{1F9E0}'}</span> ML Brain & Flywheel
          </h1>
        </div>
      ...
      <div className="mt-6 bg-[#13131a] border border-[#23232f] p-6 rounded-lg">
        <h2 className="text-2xl font-bold mb-4">{'\uD83D\uDD04'} Flywheel Learning Log (Trade Outcomes)</h2>
        <p className="text-slate-400 text-sm mb-4">Auto-recording positive trading outcomes, retraining and adjusting feature weights.</p>
        <div className="space-y-2 max-h-[300px] overflow-y-auto font-mono text-xs">
          {flywheelLogs.map((log, i) => (
            <div key={i} className={`${log.level === 'WARNING' ? 'text-amber-400' : log.level === 'SUCCESS' ? 'text-emerald-400' : 'text-slate-400'}`}>
