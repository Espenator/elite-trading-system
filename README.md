...
- [x] Fix double `council.verdict` publication (runner.py + council_gate.py) — **DONE** (intentional fallback when CouncilGate disabled)
- [x] Wire UnusualWhales flow to MessageBus so council can see it — **DONE** (unusual_whales_service.py:60)
- [ ] Start backend for first time (`uvicorn app.main:app`) — **READY** (no blockers)

### P1 — High (Blocks Full Intelligence)
- [x] Call SelfAwareness Bayesian tracking (286 lines of dead code) — **DONE** (main.py, runner.py, outcome_tracker.py)
- [x] Call IntelligenceCache.start() at startup — **DONE** (main.py:720)
- [x] Wire brain_service gRPC to hypothesis_agent — **DONE** (hypothesis_agent.py with fallback)
- [x] Establish WebSocket real-time data connectivity — **BUILT** (main.py WebSocket endpoints)
- [x] Wire 12 new Academic Edge agents into runner.py DAG stages — **DONE** (all 12 agents wired)
