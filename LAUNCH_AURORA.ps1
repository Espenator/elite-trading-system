...
Write-Host "  AURORA ELITE TRADING SYSTEM v6.0 - LAUNCHER" -ForegroundColor $COLOR_INFO
Write-Host "================================================================================" -ForegroundColor $COLOR_INFO
Write-Host ""
...
Write-Host "[6/7] Starting Aurora Dashboard..." -ForegroundColor $COLOR_INFO
...
if ($frontendRunning) {
    Write-Host "      Frontend started successfully" -ForegroundColor $COLOR_SUCCESS
} else {
    Write-Host "      WARNING: Frontend may not have started" -ForegroundColor $COLOR_WARNING
}
...
Write-Host "    Aurora Dashboard: http://localhost:$FRONTEND_PORT" -ForegroundColor $COLOR_SUCCESS
Write-Host ""
...
while ($true) {
    ...
    if ($backendAlive -and $frontendAlive) {
        Write-Host "[$timestamp] System healthy - Backend and Frontend running" -ForegroundColor $COLOR_SUCCESS
    } else {
        if (-not $backendAlive) {
            Write-Host "[$timestamp] WARNING: Backend not responding" -ForegroundColor $COLOR_ERROR
        }
        if (-not $frontendAlive) {
            Write-Host "[$timestamp] WARNING: Frontend not responding" -ForegroundColor $COLOR_ERROR
        }
    }
}
